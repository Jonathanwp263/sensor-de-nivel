...
MeshStatus_Typedef ReceivePacketCommand(uint16_t* id, uint8_t* command, uint8_t* payload, uint8_t* payloadSize, uint32_t timeout)
{
  ...
  while( ((timeout > 0 ) || (i > 0)) && (waitNextByte > 0) )
  {
    ...
    {
      // FIX 1: Ler o byte para uma variável temporária
      uint8_t byteReceived = hSerialCommand->read();
      
      // FIX 2: Proteção contra Buffer Overflow (Evita corromper a memória)
      if (i < MAX_BUFFER_SIZE) {
        frame.buffer[i++] = byteReceived;
      }
      // Se i >= MAX_BUFFER_SIZE, ignoramos o byte extra para não crashar o ESP32
  ...
  // FIX 3: Proteção contra pacotes quebrados (Integer Underflow)
  // Um pacote válido precisa ter ID(2) + CMD(1) + CRC(2) = 5 bytes no mínimo.
  // Se i < 5, a conta "i - 5" gerava um número gigante e travava o memcpy.
  if (i < 5) return MESH_ERROR;
  ...
  *payloadSize = i-5;
  ...
  /* Copies payload */
  // Só copia se houver payload real
  if (*payloadSize > 0) {
...
MeshStatus_Typedef GpioConfig(uint16_t id, GPIO_Typedef pin, Mode_Typedef mode, Pull_Typedef pull)
{
  ...
  if((pull != PULL_OFF) && (pull != PULLUP_MOTE) && (pull != PULLDOWN_MOTE)) return MESH_ERROR;
  if(hSerialCommand == NULL) return MESH_ERROR;
